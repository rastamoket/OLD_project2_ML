...
def run_2():
    ...
    :return: Nothing but create a CSV file for the submission
    '''
    ...
    #*********** Do prediction on training *************
    #******** prepare the validation set *********
    print('---- Start the predictions -----')
    train_df, train_surprise = formating_data_surprise(ratings, True) # Formating the data in order to use Surprise
    train_set = train_surprise.build_full_trainset() # Build trainset
    train_pred = train_set.build_testset() # Build iterable object in order to test

    prediction_df = train_df.copy() # Initialization of the DataFrame we will return
    ######### Predictions by the trained algorithms #############
    for i, algo_t in enumerate(algos_trained): # Loop over all the trained algorithms
        print('loop for prediction, algo: {}/{}'.format(i+1, len(algos_trained)))
        pred = algo_t.test(train_pred) # Make the prediction
    ...
    prediction_df.columns = all_col # In order to put the right name on the columns
    print('---- End of the predictions -----')

    #----------- Training the regressor on the predictions on the "train_reg_ratings" ---------------

    prediction_cleaned = prediction_df.copy() # Copy the original data, keep it intact
    prediction_cleaned = prediction_cleaned.drop(['movies ID', 'users ID'], axis = 1) # Remove the columns we don't need
    ...
    training_predictions_set, training_predictions_label = get_label_predictions(prediction_cleaned) # Take the predictions and the labels
    ...
    predicted = regressor.predict(predictions_only)#predictions_only.dot(regressor.coef_) # Compute the predictions of the unknown ratings
